...
# ---------------- MAIN ----------------
    ...
    if st.session_state.auth["role"] == "admin":
        ...
        if st.button("Create / Update School"):
        ...
        schools = run_query("SELECT * FROM schools", fetch=True) or []
        for s in schools:
            st.write(f"{s['name']} ({s['id']}) - {s['plan']}")
    ...
    # ================= SCHOOL =================
        ...
        # FEES
            ...
            for f in fees:
                st.write(f"{f['student_name']} | {f['month']} | {f['status']}")
        ...
        # INVENTORY
            ...
            if st.button("Add Item"):
                run_query(
                    ...
                    (str(uuid.uuid4()),item,"General",qty,5,sid)
                )
