    ...
    # ================= SCHOOL =================
        ...
        **Students:** {len(students)} / {max_students}
        ...
        if menu == "Dashboard":
            ...
            if bc:
                st.warning(f"📢 **Admin Broadcast:** {bc[0]['msg']}")
            ...
            with st.form("add_student"):
                ...
                if st.form_submit_button("Add Student"):
                    run_query("INSERT INTO students VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                             (str(uuid.uuid4()), name, blood, allergy, parent_name, parent_phone, likes, dislikes, siblings, student_class, sid))
                    st.success("Added!")
                    st.rerun()
                    
            for s in students:
                with st.expander(f"👶 {s['name']} - Class: {s['class']}"):
        ...
        # -------- FEES --------
            ...
            if not student_map:
                st.info("Please add students before managing fees.")
            else:
            ...
            for f in fees:
                ...
                if f["status"] == "Pending":
                    if col4.button("Mark Paid", key=f["id"]):
                        run_query("UPDATE fees SET status=?, payment_date=? WHERE id=?", 
                                 ("Paid", str(datetime.now().date()), f["id"]))
                        st.rerun()
                else:
                    col4.success("Paid")
                ...
                if st.form_submit_button("Update Stock"):
                    ...
                    if existing:
                        ...
                        run_query("INSERT INTO inventory VALUES (?, ?, ?, ?, ?, ?)", 
                                 (str(uuid.uuid4()), item_input, cat, qty, min_q, sid))
        ...
        # -------- CARE LOGS --------
            ...
            if not student_map:
                st.info("Please add students before logging activities.")
            else:
                with st.form("log"):
                    ...
                    if st.form_submit_button("Log Activity"):
                        ...
                        run_query("INSERT INTO care_logs VALUES (?, ?, ?, ?, ?, ?, ?)", 
                                 (str(uuid.uuid4()), student_map[st_name], st_name, activity, notes, t, sid))
                        st.success("Logged!")
                        st.rerun()
